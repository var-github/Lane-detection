...
if video_file is not None:
    ...
    input_video_path = os.path.join(TEMP_VIDEO_DIR, f"input_video{extension}")
    ...
    if not cap.isOpened():
        ...
        input_processed_path = os.path.join(TEMP_VIDEO_DIR, "input_processed.mp4")
        output_processed_path = os.path.join(TEMP_VIDEO_DIR, "output_processed.mp4")
        
        input_video = cv2.VideoWriter(input_processed_path, fourcc, fps, (1280, 720))
        output_video = cv2.VideoWriter(output_processed_path, fourcc, fps, (1280, 720))
        ...
        # Save paths in session state for video displaying
        st.session_state['input_processed'] = input_processed_path
        st.session_state['output_processed'] = output_processed_path
        
        st.subheader("Original Video")
        with open(st.session_state['input_processed'], 'rb') as f:
            st.video(f.read())
            
        st.subheader("Processed Video")
        with open(st.session_state['output_processed'], 'rb') as f:
            st.video(f.read())
        
        st.text(f"Saved input video path: {os.path.abspath(st.session_state['input_processed'])}")
        st.text(f"Saved output video path: {os.path.abspath(st.session_state['output_processed'])}")
